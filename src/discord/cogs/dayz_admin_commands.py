...
class DayzAdminCog(commands.Cog):
    ...
    async def set_status(self, interaction: nextcord.Interaction, status_code: int, map_name: str = "ALL", message: str = None):
        colorize_log("INFO", f"{interaction.user.name} used DayzAdminCog.set_status at {datetime.datetime.now()}")
    ...
    async def load_traderconfig(self, interaction: nextcord.Interaction) -> None:
        colorize_log("INFO", f"{interaction.user.name} used DayzAdminCog.load_traderconfig at {datetime.datetime.now()}")
    ...
    async def remove_map(self, interaction: nextcord.Interaction) -> None:
        colorize_log("WARNING", f"{interaction.user.name} used DayzAdminCog.remove_map at {datetime.datetime.now()}")
    ...
    async def get_key(self, interaction: nextcord.Interaction, mapname: str) -> None:
        colorize_log("INFO", f"{interaction.user.name} used DayzAdminCog.get_key at {datetime.datetime.now()}")
